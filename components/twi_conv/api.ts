...
export async function getToken(
  ...
  const requestAddress = "https://token-service-2280.twil.io/token-service";
  // process.env.REACT_APP_ACCESS_TOKEN_SERVICE_URL as string;
  if (!requestAddress) {
  ...
  try {
    const response = await axios.get(requestAddress, {
      params: { identity: "user00", password: "123" },
    });
