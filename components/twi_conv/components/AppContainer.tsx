...
import {addConversation, getConversationParticipants, getToken} from "../api";
...
const AppContainer: React.FC<AppCtnProps> = ({client}:AppCtnProps) => {
  /* eslint-disable */
  ...
  const lastOpenedConversation = useMemo(
      () => conversations.find((convo) => convo.sid === sid),
      [sid, conversations]
  );
  const toid=useRouter().query["id"]?.toString();
  const [openedConversation,setOpConv]=useState(toid?undefined:lastOpenedConversation);
  ...
  useEffect(()=>{
    if(toid)client.getConversationByUniqueName(toid).then((v)=>{
      setOpConv(v);})
  },[toid]);
  useEffect(() => {
